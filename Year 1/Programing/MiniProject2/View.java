...
{ 
    ...
    private int       level = 0;      // Level output
    private int       lives = 0;      // Lives output
    ...
    public void drawActualPicture( Graphics2D g )
    {
        ...
        synchronized( Model.class )   // Make thread safe
        {
            ...
            /** Set bricks to be visible **/

            for (int i = 0; i < bricks.size(); i++){
                if (bricks.get(i).isVisible()) //isVisible() returns the visible state of the brick as a boolean, i.e. isVisible(true).
                {
            ...
            String fmt = "Lvl = [%1d] Scr = [%4d] Liv = %1d fps:%5.1f";
            String text = String.format(fmt, level, score, lives,
